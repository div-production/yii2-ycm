...
(function($) {
    ...
    var methods = {
         ...
         */
        isLeaf: function() {
			if($(this).treegrid('getSetting', 'ajaxMode')) {
				if($(this).data('treegrid-is-leaf')) {
					return true;
				}
				else {
					return false;
		...
		initAjax: function () {
			...
			$this.treegrid('getAllNodes').each(function (k, el) {
				$(el).treegrid('collapse');
			});
			
			$this.on('expand', function (e) {
				var row = $(e.target);
				if(row.treegrid('getChildNodes').length) {
					return;
				}
				var expander = $(this).treegrid('getSetting', 'getExpander').apply(row);
				expander.addClass('treegrid-loading');
				
				$.ajax({
					url: '/admin/model/get-tree',
					method: 'POST',
					data: {
						treegrid_id: row.data('key'),
						model: $this.data('model')
					},
					success: function (response) {
						var html = $(response);
						html.insertAfter(row).show();
						if(!html.length) {
							row.data('treegrid-is-leaf', true);
							row.treegrid('getSetting', 'getExpander').apply(row).removeClass($this.treegrid('getSetting', 'expanderExpandedClass'));
						}
						row.removeClass('treegrid-collapsed').addClass('treegrid-expanded');
						expander.removeClass('treegrid-loading');
						html.each(function () {
							$(this).
							    data('treegrid', row.data('treegrid')).
