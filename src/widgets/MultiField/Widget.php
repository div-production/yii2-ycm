...
class Widget extends BaseWidget {
	...
	public function run() {
		...
		if(!$result) $result = $this->createRow([]);
		if($this->showCreate) $result .= '<div class="btn btn-primary js-field-add">Добавить поле</div>';
	...
	public function createRow($data) {
		...
		foreach($this->fields as $field) {
			...
			if(isset($data[$field['name']])) $options['value'] = $data[$field['name']];
			else $options['value'] = '';
			if(isset($field['hint'])) $options['placeholder'] = $field['hint'];
		...
		if($this->showRemove) $r .= '<div class="btn btn-warning js-field-remove" style="position: absolute; top: 0; left: 100%;">Удалить</div>';
